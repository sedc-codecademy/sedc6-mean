...
const repoGenerator = async () => {
    ...
    const db = await dbContext('mongodb://localhost:27017', 'bookr');
    ...
    return () => {
        ...
        const repo: AuthorRepository = {
            ...
            getAuthorById: async (id) => {
                ...
                const result = await authors.findOne({ id });
                return result;
            },
            ...
            updateAuthor: async (author) => {
                const result = await authors.findOneAndUpdate({ id: author.id }, { $set: author });
                return result.value;
            },
        }
        return repo;
